...
def test_process_emergency_event(service, sample_emergency_event):
    ...
    mock_alert = Mock()
    service.mqtt_handler._create_alert_from_event.return_value = mock_alert
    service.mqtt_handler.broadcast_alert = Mock()
    
    service.process_emergency_event(sample_emergency_event)
    
    service.mqtt_handler._create_alert_from_event.assert_called_once_with(sample_emergency_event)
